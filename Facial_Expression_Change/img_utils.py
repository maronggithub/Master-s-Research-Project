...
def mls_similarity_deformation_inv(image, p, q, alpha=1.0, density=50.0):
    ''' Similarity inverse deformation
    ### Params:
        * image - ndarray: original image
        * p - ndarray: an array with size [n, 2], original control points
...
def mls_rigid_deformation_inv(image, p, q, alpha=1.0, density=1.0):
    ...
    w[w == np.inf] = 2147483647


    pstar = np.sum(w * reshaped_p.transpose(1, 0, 2, 3), axis=1) / np.sum(w, axis=0)  # [2, grow, gcol]

    phat = reshaped_p - pstar  # [ctrls, 2, grow, gcol] windowsより遅い

    qstar = np.sum(w * reshaped_q.transpose(1, 0, 2, 3), axis=1) / np.sum(w, axis=0)  # [2, grow, gcol]windowsより遅い

    qhat = reshaped_q - qstar  # [ctrls, 2, grow, gcol]　windowsより遅い
    ...
    neg_phat_verti = phat[:, [1, 0], ...]  # [ctrls, 2, grow, gcol] windowsより早い100

    neg_phat_verti[:, 1, ...] = -neg_phat_verti[:, 1, ...]

    reshaped_neg_phat_verti = neg_phat_verti.reshape(ctrls, 1, 2, grow, gcol)  # [ctrls, 1, 2, grow, gcol]　windowsより早い100
